...
def save_insights_to_notion(insights, week_num, monday):
    """把每週偷學案例存入游泰仁靈感庫，來源標記為「游泰仁週報」"""
    INSPIRATION_DB = "352d737a-fec4-81d4-86fa-d9671a178e63"
        ...
        try:
            requests.post(
                ...
                json={
                    ...
                    "properties": {
                        "靈感內容": {"title": [{"text": {"content": content}}]},
                        "來源": {"select": {"name": "游泰仁週報"}},
                        "分類": {"select": {"name": "個人"}},
                        "觸發點": {"select": {"name": "看到的"}},
            ...
            print(f"靈感庫：寫入案例 {i+1}")
        except Exception as e:
            print(f"靈感庫寫入失敗（案例 {i+1}）：{e}")
...
<div class="layout">
  ...
  <aside class="sidebar">
    ...
    {monthly_sidebar_html}
    <div class="sidebar-section">
      <div class="sidebar-title">Threads 觀察日記</div>
      <ul class="sidebar-links">
        <li><a href="https://dunayou.github.io/weekly-report/threads/" target="_blank"><span class="week-num">📊</span>觀察總覽</a></li>
      </ul>
    </div>
    <div class="sidebar-section">
...
def main():
    ...
    if insights:
        print("寫入靈感庫...")
        save_insights_to_notion(insights, week_num, monday)
